...
Each node gets the `cdfuzzing-PG0:DedicatedMachine` image (Docker + Magma
preinstalled), a `/mydata` blockstore (default 100GB), and a static IP on a
best-effort private LAN.

### Profile parameters

| Parameter | Default | Notes |
|---|---|---|
| `osImage` | `cdfuzzing-PG0:DedicatedMachine` | prebuilt image |
...
| [cloudlab/setup-node.sh](cloudlab/setup-node.sh) | every node (boot) | mount `/mydata`, move Docker data-root there, passwordless SSH, write manifest |
