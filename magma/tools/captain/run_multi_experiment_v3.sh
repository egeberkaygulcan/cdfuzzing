#!/bin/bash
# Multi-fuzzer, multi-target 12h experiment v3
# Tests ADAPTIVE EMA stagnation threshold on 5 benchmarks
set -e
...
EXPDIR="$SCRIPT_DIR/multi_experiment_v3"
rm -rf "$EXPDIR"
mkdir -p "$EXPDIR"
...
TARGETS=(sqlite3 libtiff openssl)
SEEDS=(1 2 3)

echo "=== Multi-Fuzzer Multi-Target 12h Experiment v3 ==="
echo "ADAPTIVE EMA stagnation (alpha=0.1, factor=0.25)"
...
for pair in "${FUZZER_PAIRS[@]}"; do
    ...
    for TARGET in "${TARGETS[@]}"; do
        ...
        for SEED in "${SEEDS[@]}"; do
            ...
            # CD variant with adaptive EMA config
            launch "${CD}_${TARGET}_s${SEED}" "magma/${CD}/${TARGET}" "$SEED" "$PROGRAM" \
                ...
                --env=AFL_DRIFT_STAGNATION_FACTOR=0.25
        done
    done
done
