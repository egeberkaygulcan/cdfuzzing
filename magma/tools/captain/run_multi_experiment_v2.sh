#!/bin/bash
# Multi-fuzzer, multi-target 12h experiment v2
# Tests improved CD detection (stagnation check + cooldown) on new benchmarks
set -e
...
EXPDIR="$SCRIPT_DIR/multi_experiment_v2"
rm -rf "$EXPDIR"
mkdir -p "$EXPDIR"

# Target configs: target_name program_name
declare -A TARGET_PROGRAMS
TARGET_PROGRAMS[libpng]="libpng_read_fuzzer"
TARGET_PROGRAMS[libsndfile]="sndfile_fuzzer"
TARGET_PROGRAMS[sqlite3]="sqlite3_fuzz"
...
echo "=== Multi-Fuzzer Multi-Target 12h Experiment v2 ==="
echo "Improved CD: stagnation check + cooldown"
echo "Fuzzer pairs: afl/aflcd, aflfast/aflfastcd, moptafl/moptaflcd"
echo "Targets: libpng, libsndfile, sqlite3"
...
for pair in "${FUZZER_PAIRS[@]}"; do
    ...
    for TARGET in libpng libsndfile sqlite3; do
        ...
        for SEED in "${SEEDS[@]}"; do
            ...
            # CD variant with improved config: stagnation check + cooldown
