...
static void util_rndInitThread(void) {
    char *seed_env = getenv("FUZZER_SEED");
    if (seed_env) {
        uint64_t seed = strtoull(seed_env, NULL, 10);
        uint64_t tid = (uint64_t)(uintptr_t)pthread_self();
        rndState[0] = seed ^ (tid * 6364136223846793005ULL);
        rndState[1] = seed ^ (tid * 1442695040888963407ULL);
        if (!rndState[0] && !rndState[1]) rndState[0] = 1;
        return;
    }
